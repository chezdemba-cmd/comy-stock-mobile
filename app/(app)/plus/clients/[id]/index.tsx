...
export default function ClientDetailScreen() {
  ...
  const onRemind = () => {
    ...
    const message = buildDebtReminderMessage(customer.name, formatMoney(outstanding, currency), shopName);
      ...
      <ScrollView contentContainerStyle={styles.scroll}>
        <View style={styles.headerRow}>
          <View style={styles.avatar}>
            <Text style={styles.avatarLabel}>{customer.name.charAt(0).toUpperCase()}</Text>
          </View>
        ...
        <View style={styles.statsRow}>
          <View style={styles.statCard}>
            ...
            <Text style={styles.statValue}>{formatMoney(stats?.totalRevenue ?? 0, currency)}</Text>
          </View>
          <View style={styles.statCard}>
        ...
        <Text style={styles.lastVisit}>
          {stats?.lastVisit
            ? `Dernière visite : ${new Date(stats.lastVisit).toLocaleDateString('fr-FR')}`
        ...
        <Button
          ...
          variant="ghost"
          onPress={() => router.push(`/(app)/plus/clients/${customer.id}/edit`)}
        />
